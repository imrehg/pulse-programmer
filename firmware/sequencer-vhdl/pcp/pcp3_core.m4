...
timer_component_
  ...
  -- Timer ports for wait opcode
   signal timer_count   : pcp3_timer_constant_type;
   ...
   signal r1_address            : pcp3_register_address_type;
   signal r1_wren               : std_logic;
   signal r1_write_data         : pcp3_data_type;
   signal r1_read_data          : pcp3_data_type;
   signal r2_address            : pcp3_register_address_type;
   signal r2_read_data          : pcp3_data_type;
   ...
   signal arith_is_add          : std_logic;
   signal arith_is_mult         : std_logic;
   signal arith_is_div          : std_logic;
   signal arith_is_inc          : std_logic;
],[dnl -- Body ----------------------------------------------------------------
   -- Phase register file debug ports
  ...
  -----------------------------------------------------------------------------
  decoder : pcp3_decoder
    port map (
      wb_clk_i               => wb_clk_i,
      sclr                   => sclr,
      wait_in                => wait_in,
      instruction_in         => instruction_in,
      hard_triggers_in       => hard_triggers_in,
      firm_triggers_in       => firm_triggers_in,
      pc_out                 => pc_out,
      -- Timer ports for wait opcode
  ...
  -----------------------------------------------------------------------------
  trigger_counter1 : counter
    generic map (
      SUBCOUNTER_MULTIPLE    => 4,
      SUBCOUNTER_WIDTH       => 7
      )
    port map (
      clock      => hard_triggers_in(0),
      clk_en     => '1',
      reset      => wb_rst_i,
      sclr       => timer_clear or sclr,
      load       => timer_load,
  ...
  reg_file : pcp_reg_file
    generic map (
      DATA_WIDTH     => PCP3_DATA_WIDTH,
      ADDRESS_WIDTH  => PCP3_REGISTER_ADDRESS_WIDTH,
      REGISTER_COUNT => 2**PCP3_REGISTER_ADDRESS_WIDTH
      )
    port map (
      wb_clk_i       => wb_clk_i,
...
-------------------------------------------------------------------------------
  artithmetic : pcp_arith
    generic map (
       PCP3_DATA_WIDTH => OPERAND_WIDTH
      )
    port map (
      wb_clk_i          => clock,
      wb1_dat_o         => data_a,
      wb2_dat_o         => data_b,
      wb1_dat_i         => result,
      arith_is_add      => adder_sel,
      arith_is_mult     => mult_sel
      );
