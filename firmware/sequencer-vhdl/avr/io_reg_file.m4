...
signal sp_res : std_logic_vector(15 downto 0);
   ...
   sreg_write : process(clk, nrst, clk_en, sclr)
   begin
     if (nrst = '0') then 
       sreg <= (others => '0');
   ...
   spl_write : process(clk, nrst, clk_en, sclr)
   begin
     if (nrst = '0') then 
       spl <= (others => '0');
     elsif (rising_edge(clk)) then
   ...
   sph_write : process(clk, nrst, clk_en, sclr)
   begin
     if (nrst = '0') then 
       sph <= (others => '0');
     elsif (rising_edge(clk)) then
   ...
   rampz_write : process(clk, nrst, clk_en, sclr)
   begin
     if (nrst = '0') then 
       rampz <= (others => '0');
     elsif (rising_edge(clk)) then
